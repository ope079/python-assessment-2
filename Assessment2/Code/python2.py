...
def three(a):
	string = str(a)
	num = int(string) + int(string+string) + int(string*3) + int(string*4)
	return num
...
def four(input1, input2):
	...
	string = ''.join(''.join (i) for i in zipped)
	return string
...
def seven(a, b, c):
	...
	if list_i[0] - list_i[1] == list_i[1] - list_i[2]:
		return True
	else:
		return False
